...
subTrainSubIDs <- read.table(paste(".",subDir,subTrainSubIDFile,sep="/"),sep = " ", nrows = 10)
subTrainActIDs <- read.table(paste(".",subDir,subTrainActIDFile,sep="/"),sep = " ", nrows = 10)
subTrainData <- read.table(paste(".",subDir,subTrainDataFile,sep="/"),header = F, nrows = 10)  #note that when sep=" " is used the result is not correct
dim(subTrainData)
#expect
trainDim
