...
subTestSubIDs <- read.table(paste(".",subDir,subTestSubIDFile,sep="/"),sep = " ",nrows = 10)
subTestActIDs <- read.table(paste(".",subDir,subTestActIDFile,sep="/"),sep = " ", nrows= 10)
subTestData <- read.table(paste(".",subDir,subTestDataFile,sep="/"),header = F, nrows = 10)  #note that when sep=" " is used the result is not correct
dim(subTestData)
#expect
testDim
